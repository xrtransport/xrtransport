...
AddressSpaceStream* createGoldfishAddressSpaceStream(size_t ignored_bufSize) {
    ...
    if (!goldfish_address_space_set_subdevice_type(handle, GoldfishAddressSpaceSubdeviceType::Graphics, &child_device_handle)) {
        spdlog::error("AddressSpaceStream::create failed (initial device create)\n");
    ...
    if (!goldfish_address_space_ping(child_device_handle, &request)) {
        spdlog::error("AddressSpaceStream::create failed (get ring)\n");
    ...
    if (!goldfish_address_space_ping(child_device_handle, &request)) {
        spdlog::error("AddressSpaceStream::create failed (get buffer)\n");
        goldfish_address_space_close(child_device_handle);
        return nullptr;
    }
    ...
    if (!goldfish_address_space_claim_shared(
        ...
        spdlog::error("AddressSpaceStream::create failed (claim ring storage)\n");
        goldfish_address_space_close(child_device_handle);
        return nullptr;
    }
    ...
    if (!goldfish_address_space_claim_shared(
        ...
        spdlog::error("AddressSpaceStream::create failed (claim buffer storage)\n");
    ...
    if (!ringPtr) {
        spdlog::error("AddressSpaceStream::create failed (map ring storage)\n");
    ...
    if (!bufferPtr) {
        spdlog::error("AddressSpaceStream::create failed (map buffer storage)\n");
