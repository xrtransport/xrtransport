  ...
  : m_handle(create_address_space_fd())
{
    if ((subdevice != GoldfishAddressSpaceSubdeviceType::NoSubdevice) && is_opened()) {
        ...
        if (ret != 0 && ret != subdevice) {  // TODO: retire the 'ret != subdevice' check
            spdlog::error("{}: set_address_space_subdevice_type failed for device_type={}, ret={}",
...
{
    if (m_size == 0) {
        spdlog::error("{}: called with zero size\n", __func__);
        return NULL;
    }
    if (m_mmaped_ptr) {
        spdlog::error("'mmap' called for an already mmaped address block");
        ::abort();
    }
    ...
    if (res) {
        spdlog::error(
            "{}: host memory map failed with size 0x{:x} "
            "off 0x{:x} errno {}\n",
...
void GoldfishAddressSpaceBlock::destroy()
{
    ...
    if (m_size) {
        ...
        if (m_is_shared_mapping) {
            ...
            if (res) {
                spdlog::error("ioctl_unclaim_shared failed, res={}", res);
                ::abort();
            }
        } else {
            res = ioctl_deallocate(m_handle, m_offset);
            if (res) {
                spdlog::error("ioctl_deallocate failed, res={}", res);
                ::abort();
            }
        }
...
void GoldfishAddressSpaceHostMemoryAllocator::hostFree(GoldfishAddressSpaceBlock *block)
{
    ...
    if (!m_provider.is_opened()) {
        spdlog::error("{}: device is not available", __func__);
        ::abort();
    }
    ...
    if (block->guestPtr()) {
        ...
        if (ret) {
            spdlog::error("{}: ioctl_ping failed, ret={}", __func__, ret);
            ::abort();
        }
    }
...
void* goldfish_address_space_map(
    ...
    if (res == MAP_FAILED) {
        spdlog::error("{}: failed to map. errno: {}\n", __func__, errno);
        return 0;
    }
...
bool goldfish_address_space_ping(
    ...
    if (res) {
        spdlog::error("{}: ping failed: errno: {}\n", __func__, errno);
        return false;
    }
